package ac.limey.limeyac.player;
...
import ac.limey.limeyac.checks.impl.packetorder.PacketOrderProcessor;
import ac.limey.limeyac.events.packets.CheckManagerListener;
import ac.limey.limeyac.events.packets.PacketEntityReplication;
import ac.limey.limeyac.manager.*;
import ac.limey.limeyac.manager.player.features.FeatureManagerImpl;
import ac.limey.limeyac.manager.player.handlers.DefaultResyncHandler;
import ac.limey.limeyac.manager.player.handlers.NoOpResyncHandler;
import ac.limey.limeyac.platform.api.player.PlatformPlayer;
import ac.limey.limeyac.predictionengine.EntityFluidInteraction;
import ac.limey.limeyac.predictionengine.MovementCheckRunner;
import ac.limey.limeyac.predictionengine.PointThreeEstimator;
import ac.limey.limeyac.predictionengine.UncertaintyHandler;
import ac.limey.limeyac.manager.AttackCooldownHandler;
import ac.limey.limeyac.predictionengine.blockeffects.CompensatedGeysers;
...
import ac.limey.limeyac.utils.nmsutil.Materials;
import ac.limey.limeyac.utils.nmsutil.StuckSpeed;
import ac.limey.limeyac.utils.viaversion.ViaVersionUtil;
...
public class LimeyPlayer implements GrimUser {
    ...
    @Getter @Setter private ResyncHandler resyncHandler = LimeyAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("disable-default-resync-handler", false) ? NoOpResyncHandler.INSTANCE : new DefaultResyncHandler(this);
    ...
    // This variable is for support with test servers that want to be able to disable limey
    // Limey disabler 2022 still working!
    public boolean disableLimey;
    ...
    public LimeyPlayer(@NotNull User user) {
    ...
    public void onPacketCancel() {
        if (spamThreshold != -1 && cancelledPackets.incrementAndGet() > spamThreshold) {
            ...
            disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, LimeyAPI.INSTANCE.getConfigManager().getDisconnectClosed())));
    ...
    public void sendTransaction(boolean async) {
        ...
        if (disableLimey && (System.nanoTime() - getPlayerClockAtLeast()) > 15e9) {
            return;
        }
    ...
    public void timedOut() {
        disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, LimeyAPI.INSTANCE.getConfigManager().getDisconnectTimeout())));
    }
    ...
    public void disconnect(Component reason) {
        ...
        if (platformPlayer != null) {
            LimeyAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, LimeyAPI.INSTANCE.getLimeyPlugin(),
    ...
    public void pollData() {
        ...
        if (!LimeyAPI.INSTANCE.getPlayerDataManager().shouldCheck(user)) {
            LimeyAPI.INSTANCE.getPlayerDataManager().remove(user);
        }
        ...
        if (this.platformPlayer == null) {
            this.platformPlayer = LimeyAPI.INSTANCE.getPlatformPlayerFactory().getFromUUID(uuid);
            updatePermissions();
        }
        ...
        LimeyAPI.INSTANCE.getDataStoreLifecycle().sessionTracker()
    ...
    public void updatePermissions() {
        runSafely(() -> {
            try {
                boolean noModifyPacketPermission = hasPermission("limey.nomodifypacket");
                boolean noSetbackPermission = hasPermission("limey.nosetback");
                boolean disabledPermission = hasPermission("limey.disabled");
                boolean exemptPermission = hasPermission("limey.exempt");
                ...
                this.disableLimey = disabledPermission;
                if (exemptPermission) {
    ...
    public int getTransactionPing() {
        return LimeyMath.floor(transactionPing / 1e6);
    }
    ...
    public final void reload(ConfigManager config) {
        ...
        maxTransactionTime = LimeyMath.clamp(config.getIntElse("max-transaction-time", 60), 1, 180);
    ...
    public void resyncPositions(SimpleCollisionBox box) {
        this.resyncHandler.resync(LimeyMath.floor(box.minX), LimeyMath.floor(box.minY), LimeyMath.floor(box.minZ),
                LimeyMath.ceil(box.maxX), LimeyMath.ceil(box.maxY), LimeyMath.ceil(box.maxZ));
    }

    public void addMovementThisTick(@NotNull LimeyPlayer.Movement movement) {
        Objects.requireNonNull(movement, "movement");

        if (this.movementThisTick.size() >= 100) {
            LimeyPlayer.Movement movement1 = this.movementThisTick.removeFirst();
            LimeyPlayer.Movement movement2 = this.movementThisTick.removeFirst();
            LimeyPlayer.Movement movement3 = new LimeyPlayer.Movement(movement1.from(), movement2.to());
