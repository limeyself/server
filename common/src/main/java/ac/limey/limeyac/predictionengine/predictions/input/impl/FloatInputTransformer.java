package ac.limey.limeyac.predictionengine.predictions.input.impl;

import ac.limey.limeyac.player.LimeyPlayer;
import ac.limey.limeyac.predictionengine.predictions.input.FloatInput;
...
public class FloatInputTransformer implements InputTransformer<FloatInput> { // 1.7 - 1.13.2
    @Override
    public FloatInput transformInputsToVector(LimeyPlayer player, int sideways, int vertical, int forward) {
        float bestPossibleX;
        float bestPossibleZ;
        ...
        if (player.isSlowMovement) {
            ...
            bestPossibleX = LimeyMath.clamp(sideways, -1, 1);
            bestPossibleZ = LimeyMath.clamp(forward, -1, 1);
        }
    ...
    @Override
    public Vector3dm getMovementResultFromInput(LimeyPlayer player, Input inputVector, float speed, float yaw) {
        ...
        if (lengthSquared >= 1.0E-4F) {
            lengthSquared = LimeyMath.sqrt(lengthSquared);
            ...
            float yawRadians = LimeyMath.radians(yaw);
