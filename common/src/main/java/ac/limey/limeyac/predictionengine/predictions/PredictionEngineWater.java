...
public class PredictionEngineWater extends PredictionEngine {
    ...
    public static void staticVectorEndOfTick(LimeyPlayer player, Vector3dm vector, float swimmingFriction, double playerGravity, boolean isFalling) {
    ...
    public static Set<VectorData> transformSwimmingVectors(LimeyPlayer player, Set<VectorData> base) {
    ...
    public void guessBestMovement(float swimmingSpeed, LimeyPlayer player, boolean isFalling, double playerGravity, float swimmingFriction) {
    ...
    public void addJumpsToPossibilities(LimeyPlayer player, Set<VectorData> existingVelocities) {
        ...
        for (VectorData vector : new HashSet<>(existingVelocities)) {
            if (player.couldSkipTick && vector.isZeroPointZeroThree()) {
                double extraVelFromVertTickSkipUpwards = LimeyMath.clamp(player.actualMovement.getY(), vector.vector.clone().getY(), vector.vector.clone().getY() + 0.05f);
