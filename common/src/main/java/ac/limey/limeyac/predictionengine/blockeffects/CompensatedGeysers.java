...
public class CompensatedGeysers {
    ...
    public List<GeyserBlockEntity> getTickersInOrder(LimeyPlayer player, int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
        if (SERVER_VERSION.isOlderThan(ClientVersion.V_26_2)) {
            return null;
        }
    ...
    private void addTicker(int x, int y, int z, int minHeight) {
        ...
        long pos = LimeyMath.asLong(x, y, z);
    ...
    private void removeFromSection(int x, int y, int z, long sectionKey) {
        ...
        if (section != null) {
            section.remove(LimeyMath.asLong(x, y, z));
