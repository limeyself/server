...
import ac.limey.limeyac.utils.data.VelocityData;
 ...
 * Someone trying to false limey by using negative levitation effects
...
public class PointThreeEstimator {
    ...
    public PointThreeEstimator(LimeyPlayer player) {
        this.player = player;
    }
    ...
    public void handleChangeBlock(int x, int y, int z, WrappedBlockState state) {
        ...
        if (pointThreeBox.isIntersected(new SimpleCollisionBox(x, y, z))) {
            // https://github.com/MWHunter/Limey/issues/613
    ...
    private double iterateGravity(LimeyPlayer player, double y) {
