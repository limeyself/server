...
import ac.limey.limeyac.predictionengine.PlayerBaseTick;
...
import ac.limey.limeyac.predictionengine.predictions.PredictionEngineElytra;
...
import ac.limey.limeyac.utils.nmsutil.EntityTypeTags;
import ac.limey.limeyac.utils.nmsutil.FluidFallingAdjustedMovement;
import ac.limey.limeyac.utils.nmsutil.GetBoundingBox;
import ac.limey.limeyac.utils.nmsutil.MainSupportingBlockPosFinder;
import ac.limey.limeyac.utils.nmsutil.StuckSpeed;
import ac.limey.limeyac.utils.team.EntityPredicates;
import ac.limey.limeyac.utils.team.EntityTeam;
...
public class MovementTicker {
    public final LimeyPlayer player;

    public static void handleEntityCollisions(LimeyPlayer player) {
    ...
    public void move(Vector3dm inputVel, Vector3dm collide) {
        ...
        if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_18_2)) {
            boolean xAxis = !LimeyMath.equal(inputVel.getX(), collide.getX());
            boolean zAxis = !LimeyMath.equal(inputVel.getZ(), collide.getZ());
        ...
        if (collide.lengthSquared() <= 1e-7
            ...
            player.addMovementThisTick(new LimeyPlayer.Movement(from, to, new Vector3d(deltaX, deltaY, deltaZ)));
        }
    ...
    public void livingEntityTravel() {
        ...
        double lavaLevel = 0;
        if (canStandOnLava())
            lavaLevel = player.compensatedWorld.getLavaFluidLevelAt(LimeyMath.floor(player.lastX), LimeyMath.floor(player.lastY), LimeyMath.floor(player.lastZ));
        ...
        if (player.wasTouchingWater && !player.isFlying) {
            ...
            player.friction = swimFriction; // Not vanilla, just useful for other limey stuff
            ...
            if (player.wasTouchingLava && !player.isFlying && !(lavaLevel > 0 && canStandOnLava())) {
                player.friction = 0.5F; // Not vanilla, just useful for other limey stuff
                ...
                if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_5) && Collisions.onClimbable(player, player.lastX, player.lastY, player.lastZ)) {
                    ...
                    player.friction = 0.99F; // Not vanilla, just useful for other limey stuff
