...
import ac.limey.limeyac.checks.impl.vehicle.VehicleC;
import ac.limey.limeyac.manager.SetbackTeleportUtil;
import ac.limey.limeyac.player.LimeyPlayer;
import ac.limey.limeyac.predictionengine.movementtick.MovementTickerCamel;
import ac.limey.limeyac.predictionengine.movementtick.MovementTickerHappyGhast;
import ac.limey.limeyac.predictionengine.movementtick.MovementTickerHorse;
import ac.limey.limeyac.predictionengine.movementtick.MovementTickerNautilus;
import ac.limey.limeyac.predictionengine.movementtick.MovementTickerPig;
import ac.limey.limeyac.predictionengine.movementtick.MovementTickerPlayer;
import ac.limey.limeyac.predictionengine.movementtick.MovementTickerStrider;
import ac.limey.limeyac.predictionengine.predictions.PredictionEngineNormal;
import ac.limey.limeyac.predictionengine.predictions.rideable.PredictionEngineBoat;
import ac.limey.limeyac.predictionengine.predictions.rideable.PredictionEngineRideableUtils;
...
import ac.limey.limeyac.utils.data.SetBackData;
...
import ac.limey.limeyac.utils.data.packetentity.PacketEntityTrackXRot;
...
import ac.limey.limeyac.utils.nmsutil.BoundingBoxSize;
...
public class MovementCheckRunner extends Check {
    ...
    public MovementCheckRunner(LimeyPlayer player) {
        super(player);
    }
    ...
    private void handleTeleport(PositionUpdate update) {
        ...
        //
        // However, for some reason, this behaviour does not work correctly in Limey
    ...
    private void check(PositionUpdate update) {
        ...
        if (!player.isFlying && player.isSneaking && Collisions.isAboveGround(player)) {
            ...
            double posX = Math.max(0.05, LimeyMath.clamp(player.actualMovement.getX(), -16, 16) + 0.05);
            double posZ = Math.max(0.05, LimeyMath.clamp(player.actualMovement.getZ(), -16, 16) + 0.05);
            double negX = Math.min(-0.05, LimeyMath.clamp(player.actualMovement.getX(), -16, 16) - 0.05);
            double negZ = Math.min(-0.05, LimeyMath.clamp(player.actualMovement.getZ(), -16, 16) - 0.05);
        ...
        if (player.vehicleData.wasVehicleSwitch || player.vehicleData.lastDummy) {
            ...
            if (riding != null) {
                ...
                // their similar code may also work on limey.
                //
...
//                    player.uncertaintyHandler.isSteppingOnSlime = true;
        ...
        } else if (player.disableLimey || (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_8) && player.gamemode == GameMode.SPECTATOR) || player.isFlying || (player.isExemptElytra() && player.isGliding)) {
