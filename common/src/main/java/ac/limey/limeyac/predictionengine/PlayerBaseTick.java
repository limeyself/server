package ac.limey.limeyac.predictionengine;

import ac.limey.limeyac.player.LimeyPlayer;
import ac.limey.limeyac.utils.collisions.datatypes.SimpleCollisionBox;
import ac.limey.limeyac.utils.data.attribute.ValuedAttribute;
...
import ac.limey.limeyac.utils.latency.CompensatedEntities;
...
import ac.limey.limeyac.utils.nmsutil.FluidTypeFlowing;
...
public final class PlayerBaseTick {
    ...
    public static boolean canEnterPose(LimeyPlayer player, Pose pose, double x, double y, double z) {
        return Collisions.isEmpty(player, getBoundingBoxForPose(player, pose, x, y, z).expand(-1.0E-7D));
    }

    private static @NotNull SimpleCollisionBox getBoundingBoxForPose(LimeyPlayer player, Pose pose, double x, double y, double z) {
    ...
    public static void doBaseTick(LimeyPlayer player) {
    ...
    private static void updateFluidOnEyes(LimeyPlayer player) {
    ...
    private static void updateInWaterStateAndDoFluidPushing(LimeyPlayer player) {
    ...
    public static void updatePowderSnow(LimeyPlayer player) {
    ...
    // 1.14
    public static void updatePlayerPose(LimeyPlayer player) {
    ...
    // 1.13 and below
    private static void updatePlayerSize(LimeyPlayer player) {
        Pose pose;
        if (player.isGliding) {
            pose = Pose.FALL_FLYING;
        } else if (player.isInBed) {
    ...
    private static void updateSwimming(LimeyPlayer player) {
    ...
    private static void moveTowardsClosestSpace(LimeyPlayer player, double xPosition, double zPosition) {
    ...
    private static void moveTowardsClosestSpaceLegacy(LimeyPlayer player, double x, double z) {
        int floorX = LimeyMath.floor(x);
        int floorZ = LimeyMath.floor(z);
        int floorY = LimeyMath.floor(player.lastY + 0.5);
    ...
    // 1.14+
    private static void moveTowardsClosestSpaceModern(LimeyPlayer player, double xPosition, double zPosition) {
    ...
    public static void updateInWaterStateAndDoWaterCurrentPushing(LimeyPlayer player) {
        final PacketEntity riding = player.compensatedEntities.self.getRiding();
    ...
    private static boolean updateFluidHeightAndDoFluidPushing(LimeyPlayer player, FluidTag tag, double multiplier) {
    ...
    private static boolean updateFluidHeightAndDoFluidPushingLegacy(LimeyPlayer player, FluidTag tag, double multiplier) {
        SimpleCollisionBox aABB = player.boundingBox.copy().expand(0, -0.4, 0).expand(-0.001);

        int floorX = LimeyMath.floor(aABB.minX);
        int ceilX = LimeyMath.ceil(aABB.maxX);
        int floorY = LimeyMath.floor(aABB.minY);
        int ceilY = LimeyMath.ceil(aABB.maxY);
        int floorZ = LimeyMath.floor(aABB.minZ);
        int ceilZ = LimeyMath.ceil(aABB.maxZ);
    ...
    private static boolean updateFluidHeightAndDoFluidPushingModern(LimeyPlayer player, FluidTag tag, double multiplier) {
    ...
    public static boolean updateFluidInteraction(LimeyPlayer player) {
    ...
    private static boolean suffocatesAt(LimeyPlayer player, int x, int z) {
    ...
    private static boolean clearAbove(LimeyPlayer player, int x, int y, int z) {
