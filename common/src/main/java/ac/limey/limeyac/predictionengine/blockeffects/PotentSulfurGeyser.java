...
public class PotentSulfurGeyser {
    ...
    public static void launchEntityTicker(LimeyPlayer player, Vector3dm clientVelocity, boolean checkFallDistanceAccumulation) {
    ...
    private static void launchFromPotentSulfurGeysers(LimeyPlayer player, Vector3dm clientVelocity, boolean checkFallDistanceAccumulation) {
        SimpleCollisionBox box = player.boundingBox;

        int minX = LimeyMath.floor(box.minX);
        int maxX = LimeyMath.floor(box.maxX);
        int minY = Math.max(player.compensatedWorld.getMinHeight(), LimeyMath.floor(box.minY) - 25);
        int maxY = Math.min(player.compensatedWorld.getMaxHeight() - 1, LimeyMath.floor(box.maxY) - 1);
        int minZ = LimeyMath.floor(box.minZ);
        int maxZ = LimeyMath.floor(box.maxZ);
    ...
    private static Vector3i findNoxiousGasSourceBlock(LimeyPlayer player, int x, int y, int z) {
        int maxY = y + 5;
    ...
    private static int getUnobstructedBlockCount(LimeyPlayer player, int x, int y, int z, int waterBlocks) {
    ...
    private static boolean isGeyserPassableBlock(LimeyPlayer player, WrappedBlockState state, int x, int y, int z, int geyserPositionY) {
    ...
    private static void checkFallDistanceAccumulation(LimeyPlayer player, Vector3dm clientVelocity) {
