...
import ac.limey.limeyac.checks.impl.badpackets.BadPacketsB;
import ac.limey.limeyac.player.LimeyPlayer;
import ac.limey.limeyac.utils.anticheat.update.*;
import ac.limey.limeyac.utils.blockplace.BlockPlaceResult;
import ac.limey.limeyac.utils.blockplace.ConsumesBlockPlace;
import ac.limey.limeyac.utils.change.BlockModification;
...
import ac.limey.limeyac.utils.nmsutil.*;
...
public class CheckManagerListener extends PacketListenerAbstract {
    ...
    private static void placeWaterLavaSnowBucket(LimeyPlayer player, ItemStack held, StateType toPlace, InteractionHand hand, int sequence) {
    ...
    public static void handleQueuedPlaces(LimeyPlayer player, boolean hasLook, float pitch, float yaw, long now) {
    ...
    public static void handleQueuedBreaks(LimeyPlayer player, boolean hasLook, float pitch, float yaw, long now) {
    ...
    private static void handleUseItem(LimeyPlayer player, ItemStack placedWith, InteractionHand hand, int sequence) {
    ...
    private static void handleBlockPlaceOrUseItem(PacketWrapper<?> packet, LimeyPlayer player) {
    ...
    private static void placeBucket(LimeyPlayer player, InteractionHand hand, int sequence) {
    ...
    public static void setPlayerItem(LimeyPlayer player, InteractionHand hand, ItemType type) {
    ...
    private static void placeLilypad(LimeyPlayer player, InteractionHand hand, int sequence) {
        HitData data = WorldRayTrace.getNearestBlockHitResult(player, null, true, false, true);
    ...
    private static boolean isDuplicatePacket(@NotNull LimeyPlayer player, @NotNull WrapperPlayClientPlayerFlying flying) {
    ...
    private static void handleDuplicatePacket(
        ...
        // Additionally, only yaw and pitch matters: https://github.com/LimeyAnticheat/Limey/issues/1275#issuecomment-1872444018
    ...
    private static void handleFlying(LimeyPlayer player, double x, double y, double z, float yaw, float pitch, boolean hasPosition, boolean hasLook, boolean onGround, TeleportAcceptData teleportData) {
    ...
    private static void handleDigging(LimeyPlayer player, PacketReceiveEvent event) {
            ...
            if (damage >= 1) {
                ...
                player.blockHistory.add(
                        new BlockModification(
                                ...
                                blockBreak.position,
                                LimeyAPI.INSTANCE.getTickManager().currentTick,
