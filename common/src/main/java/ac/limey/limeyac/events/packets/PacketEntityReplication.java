...
import ac.limey.limeyac.utils.data.TrackerData;
import ac.limey.limeyac.utils.data.packetentity.DashableEntity;
import ac.limey.limeyac.utils.data.packetentity.PacketEntity;
import ac.limey.limeyac.utils.data.packetentity.PacketEntityHook;
import ac.limey.limeyac.utils.data.packetentity.PacketEntityTrackXRot;
import ac.limey.limeyac.utils.enums.Pose;
import ac.limey.limeyac.utils.nmsutil.EntityMetadataPoseUtil;
...
public class PacketEntityReplication extends Check implements PacketReceiveListener, PacketSendListener {
    ...
    public PacketEntityReplication(LimeyPlayer player) {
        super(player);
    }
    ...
    public void onPacketSend(PacketSendEvent event) {
        // ensure limey is the one that sent the transaction
            ...
            if (slot.getWindowId() == 0) {
                Runnable task = () -> {
                    if (slot.getSlot() - 36 == player.packetStateData.lastSlotSelected && (
                        ...
                        if (player.isResetItemUsageOnItemUpdate() && hand == LimeyAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer)) {
                            LimeyAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                        }
                    }
                };
                ...
                Runnable task = () -> {
                    if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_8)) {
                        ...
                        if (items.getItems().size() > 45 && !player.inventory.getOffHand().is(items.getItems().get(45).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && LimeyAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.OFF_HAND) {
                                LimeyAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                        ...
                        if (!player.inventory.getHeldItem().is(items.getItems().get(player.packetStateData.lastSlotSelected + 36).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && LimeyAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.MAIN_HAND) {
                                LimeyAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                    }
        ...
        // 1.8 clients fail to send the RELEASE_USE_ITEM packet when a window is opened client sided while using an item
            ...
            if (!attach.isLeash()) {
                // Alright, let's convert this to the 1.9+ format to make it easier for limey
    ...
    private void handleMoveEntity(PacketSendEvent event, int entityId, double deltaX, double deltaY, double deltaZ, Float yaw, Float pitch, boolean isRelative, boolean hasPos) {
        ...
        if (data != null) {
            ...
            if (isRelative) {
                ...
                // This causes impossible hits, so limey must replace this with a teleport entity packet
    ...
    private boolean isDirectlyAffectingPlayer(LimeyPlayer player, int entityID) {
        // The attributes for this entity is active, currently
