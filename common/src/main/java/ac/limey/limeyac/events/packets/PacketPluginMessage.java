...
public class PacketPluginMessage extends PacketListenerAbstract {
    ...
    private void checkChannel(User user, String channelName) {
        ...
        if (usingProxy) {
            LogUtil.warn(
                    user.getName() + " seems to have connected through a proxy running ViaVersion. "
                            + "Having ViaVersion installed on the proxy is incompatible with Limey and causes many issues. "
        ...
        if (CommonLimeyArguments.KICK_ON_VIA_PROXY.value() && (!usingProxy || ViaVersionUtil.isAvailable)) {
            ...
            try {
                WrapperPlayServerDisconnect disconnect = new WrapperPlayServerDisconnect(
                        MessageUtil.miniMessage(LimeyAPI.INSTANCE.getConfigManager().getDisconnectPacketError())
                );
