...
import ac.limey.limeyac.utils.data.ShortToLongPair;
...
public class PacketPingListener extends PacketListenerAbstract {
    ...
    private static final GrimTransactionSendEvent.Channel SEND_CHANNEL = LimeyAPI.INSTANCE.getEventBus().get(GrimTransactionSendEvent.class);
    private static final GrimTransactionReceivedEvent.Channel RECEIVED_CHANNEL = LimeyAPI.INSTANCE.getEventBus().get(GrimTransactionReceivedEvent.class);
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        if (event.getPacketType() == PacketType.Play.Client.WINDOW_CONFIRMATION) {
            ...
            if (id <= 0 && player.addTransactionResponse(id)) {
                ...
                boolean shouldCancel = !LimeyAPI.INSTANCE.getConfigManager().isDisablePongCancelling();
