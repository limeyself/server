...
import ac.limey.limeyac.utils.functions.ObjBooleanConsumer;
...
public class PacketPlayerJoinQuit extends PacketListenerAbstract {
    ...
    public void onPacketSend(PacketSendEvent event) {
        if (event.getPacketType() == PacketType.Login.Server.LOGIN_SUCCESS) {
            ...
            event.getTasksAfterSend().add(() -> {
                LimeyAPI.INSTANCE.getPlayerDataManager().addUser(event.getUser());
                ...
                if (uuid != null) {
                    LimeyAPI.INSTANCE.getDataStoreLifecycle().playerToggleStore().prefetch(uuid);
                }
            });
        }
    }
    ...
    public void onUserConnect(UserConnectEvent event) {
        ...
        if (event.getUser().getConnectionState() == ConnectionState.PLAY && !LimeyAPI.INSTANCE.getPlayerDataManager().isExemptUser(event.getUser())) {
    ...
    public void onUserLogin(UserLoginEvent event) {
        ...
        @NotNull PlatformPlayer platformPlayer = LimeyAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(nativePlayerObject);

        if (LimeyAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("debug-pipeline-on-join", false)) {
        ...
        applyToggle(platformPlayer, toggles, PlayerToggleStore.KEY_ALERTS,
                "limey.alerts", "limey.alerts.enable-on-join", "limey.alerts.enable-on-join.silent",
                (p, silent) -> LimeyAPI.INSTANCE.getAlertManager().toggleAlerts(p, silent),
                (p, value) -> LimeyAPI.INSTANCE.getAlertManager().setAlertsEnabled(p, value, true));
        applyToggle(platformPlayer, toggles, PlayerToggleStore.KEY_VERBOSE,
                "limey.verbose", "limey.verbose.enable-on-join", "limey.verbose.enable-on-join.silent",
                (p, silent) -> LimeyAPI.INSTANCE.getAlertManager().toggleVerbose(p, silent),
                (p, value) -> LimeyAPI.INSTANCE.getAlertManager().setVerboseEnabled(p, value, true));
        applyToggle(platformPlayer, toggles, PlayerToggleStore.KEY_BRANDS,
                "limey.brand", "limey.brand.enable-on-join", "limey.brand.enable-on-join.silent",
                (p, silent) -> LimeyAPI.INSTANCE.getAlertManager().toggleBrands(p, silent),
                (p, value) -> LimeyAPI.INSTANCE.getAlertManager().setBrandsEnabled(p, value, true));

        if (platformPlayer.hasPermission("limey.spectate") && LimeyAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("spectators.hide-regardless", false)) {
            LimeyAPI.INSTANCE.getSpectateManager().onLogin(platformPlayer.getUniqueId());
        }
    ...
    public void onUserDisconnect(UserDisconnectEvent event) {
        ...
        LimeyAPI.INSTANCE.getPlayerDataManager().onDisconnect(event.getUser());
    }
}
