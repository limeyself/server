package ac.limey.limeyac.platform.luckperms;
...
public abstract class AbstractLuckPermsHandler implements StoppableInitable {
    ...
    private void onNodeMutate(PermissionHolder target, Node node) {
        if (target instanceof User user) {
            if (shouldRefreshUserForNode(node)) refreshLimeyPlayerPermissions(user);
            return;
        }
    ...
    private void onUserDataRecalculate(UserDataRecalculateEvent event) {
        refreshLimeyPlayerPermissions(event.getUser());
    }
    ...
    private void onContextUpdate(ContextUpdateEvent event) {
        refreshLimeyPlayerPermissions(contextSubjectUuid(event));
    }

    private boolean shouldRefreshUserForNode(Node node) {
        if (node instanceof PermissionNode permissionNode) return isLimeyPermission(permissionNode);
        if (node instanceof InheritanceNode inheritanceNode) return inheritedGroupMayAffectLimeyPermissions(inheritanceNode);
        return false;
    }
    ...
    private static boolean isLimeyPermission(PermissionNode node) {
        String permission = node.getPermission().toLowerCase(Locale.ROOT);
        return permission.equals("*") || permission.equals("limey") || permission.startsWith("limey.");
    }

    private boolean inheritedGroupMayAffectLimeyPermissions(InheritanceNode node) {
        if (luckPerms == null) return true;
        Group group = luckPerms.getGroupManager().getGroup(node.getGroupName());
        // Refresh if the inherited group was deleted, or if that group can grant Limey permissions.
        return group == null || holderHasLimeyPermission(group, new LinkedHashSet<>());
    }

    private static boolean holderHasLimeyPermission(PermissionHolder holder, Set<String> seenGroups) {
        for (Node node : holder.getNodes()) {
            if (node instanceof PermissionNode permissionNode && isLimeyPermission(permissionNode)) return true;
        }

        for (Group group : holder.getInheritedGroups(holder.getQueryOptions())) {
            if (!seenGroups.add(normalizeGroupName(group))) continue;
            if (holderHasLimeyPermission(group, seenGroups)) return true;
        }
    ...
    private static void refreshLimeyPlayerPermissions(UUID uuid) {
        if (uuid == null) return;
        LimeyPlayer player = LimeyAPI.INSTANCE.getPlayerDataManager().getPlayer(uuid);
        if (player == null) return;
        player.updatePermissions();
    }

    private static void refreshLimeyPlayerPermissions(User user) {
        if (user == null) return;
        refreshLimeyPlayerPermissions(user.getUniqueId());
    }
}
