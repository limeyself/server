package ac.limey.limeyac.utils.anticheat.update;

import ac.limey.limeyac.player.LimeyPlayer;
import ac.limey.limeyac.utils.anticheat.LogUtil;
import ac.limey.limeyac.utils.collisions.AxisSelect;
...
public class BlockPlace {
    ...
    public BlockPlace(LimeyPlayer player, InteractionHand hand, Vector3i position, int faceId, BlockFace face, ItemStack itemStack, @Nullable HitData hitData, int sequence) {
        this.player = player;
        this.hand = hand;
    ...
    private List<BlockFace> getNearestLookingDirections() {
        float pitch = LimeyMath.radians(player.pitch);
        float yaw = LimeyMath.radians(-player.yaw);
    ...
    public BlockFace getPlayerFacing() {
        return BY_2D[LimeyMath.floor(player.yaw / 90.0D + 0.5D) & 3];
    }
