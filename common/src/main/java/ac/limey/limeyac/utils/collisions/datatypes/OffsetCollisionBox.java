...
//        }
    ...
    public SimpleCollisionBox offset(double x, double y, double z) {
        ...
        return switch (offsetType) {
            ...
            case XZ -> {
                long l = LimeyMath.hashCode(x, 0, z);
                offsetX = LimeyMath.clamp(((double) ((float) (l & 15L) / 15.0F) - 0.5) * 0.5, -maxHorizontalModelOffset, maxHorizontalModelOffset);
                offsetZ = LimeyMath.clamp(((double) ((float) (l >> 8 & 15L) / 15.0F) - 0.5) * 0.5, -maxHorizontalModelOffset, maxHorizontalModelOffset);
