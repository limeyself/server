package ac.limey.limeyac.utils.data.attribute;
...
import static ac.limey.limeyac.utils.latency.CompensatedEntities.SPRINTING_MODIFIER_UUID;
...
public final class ValuedAttribute {
     ...
     */
    public ValuedAttribute requiredVersion(LimeyPlayer player, ClientVersion requiredVersion) {
    ...
    public double with(WrapperPlayServerUpdateAttributes.Property property) {
        ...
        double newValue = LimeyMath.clamp((baseValue + additionSum) * (1 + multiplyBaseSum) * multiplyTotalProduct, min, max);
        if (setRewriter != null) {
