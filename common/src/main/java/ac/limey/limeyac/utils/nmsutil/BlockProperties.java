...
import ac.limey.limeyac.utils.collisions.datatypes.CollisionBox;
import ac.limey.limeyac.utils.collisions.datatypes.ComplexCollisionBox;
import ac.limey.limeyac.utils.collisions.datatypes.SimpleCollisionBox;
import ac.limey.limeyac.utils.data.MainSupportingBlockData;
import ac.limey.limeyac.utils.data.packetentity.PacketEntity;
import ac.limey.limeyac.utils.data.packetentity.PacketEntityHorse;
import ac.limey.limeyac.utils.data.packetentity.PacketEntityNautilus;
import ac.limey.limeyac.utils.data.packetentity.PacketEntityStrider;
...
public class BlockProperties {
    public static float getFrictionInfluencedSpeed(float blockFriction, LimeyPlayer player) {
     ...
     */
    public static StateType getOnPos(LimeyPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static float getFriction(LimeyPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)) {
    ...
    public static float getBlockSpeedFactor(LimeyPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static boolean onHoneyBlock(LimeyPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
     ...
     */
    private static StateType getBlockPosBelowThatAffectsMyMovement(LimeyPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        Vector3i pos = player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)
                ? new Vector3i(LimeyMath.floor(playerPos.getX()), LimeyMath.floor(playerPos.getY() - 0.5000001), LimeyMath.floor(playerPos.getZ()))
                : getOnPos(player, playerPos, mainSupportingBlockData, 0.500001F);
        return player.compensatedWorld.getBlockType(pos.x, pos.y, pos.z);
    }

    private static Vector3i getOnPos(LimeyPlayer player, Vector3d playerPos, MainSupportingBlockData mainSupportingBlockData, float searchBelowPlayer) {
        ...
        if (mainBlockPos != null) {
            ...
            return shouldReturn ? mainBlockPos.withY(LimeyMath.floor(playerPos.getY() - (double) searchBelowPlayer)) : mainBlockPos;
        } else {
            return new Vector3i(LimeyMath.floor(playerPos.getX()), LimeyMath.floor(playerPos.getY() - searchBelowPlayer), LimeyMath.floor(playerPos.getZ()));
        }
    }

    public static float getMaterialFriction(LimeyPlayer player, StateType material) {
        float friction = 0.6f;
    ...
    private static StateType getOnBlock(LimeyPlayer player, double x, double y, double z) {
        StateType block1 = player.compensatedWorld.getBlockType(LimeyMath.floor(x), LimeyMath.floor(y - 0.2F), LimeyMath.floor(z));

        if (block1.isAir()) {
            StateType block2 = player.compensatedWorld.getBlockType(LimeyMath.floor(x), LimeyMath.floor(y - 1.2F), LimeyMath.floor(z));
    ...
    private static float getBlockSpeedFactorLegacy(LimeyPlayer player, Vector3d pos) {
    ...
    private static float getBlockSpeedFactor(LimeyPlayer player, StateType type) {
    ...
    private static float getModernVelocityMultiplier(LimeyPlayer player, float blockSpeedFactor) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) return blockSpeedFactor;
        return (float) LimeyMath.lerp((float) player.compensatedEntities.self.getAttributeValue(Attributes.MOVEMENT_EFFICIENCY), blockSpeedFactor, 1.0F);
    }

    public static float getModifiedFriction(float friction, LimeyPlayer player) {
        ...
        return LimeyMath.clamp(1.0F - (1.0F - friction) * (float) entity.getAttributeValue(Attributes.FRICTION_MODIFIER), 0.0F, 1.0F);
    }

    public static float getModifiedAirDrag(float drag, LimeyPlayer player) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_26_2)) {
            return drag;
        }

        PacketEntity entity = player.inVehicle() ? player.compensatedEntities.self.getRiding() : player.compensatedEntities.self;
        return LimeyMath.clamp(1.0F - (1.0F - drag) * (float) entity.getAttributeValue(Attributes.AIR_DRAG_MODIFIER), 0.0F, 1.0F);
    }

    public static float getEntityBounciness(LimeyPlayer player) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_26_2)) {
            return 0.0F;
        }
    ...
    public static double getVelocityAfterHorizontalCollision(LimeyPlayer player, double velocity) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_26_2)) {
            return 0.0;
        }
    ...
    public static double getVelocityAfterVerticalCollision(LimeyPlayer player, double velocity, double movementY) {
        return getVelocityAfterVerticalCollision(player, velocity, movementY, getEntityBounciness(player));
    }

    public static double getVelocityAfterVerticalCollision(LimeyPlayer player, double velocity, double movementY, double restitution) {
        ...
        double effectiveDrag = LimeyMath.lerp(portionWithMovement, 1.0, getModifiedAirDrag(0.98F, player));
        return (gravityCompensation - velocity) * effectiveDrag * restitution;
    }

    public static double getEffectiveGravity(LimeyPlayer player, double deltaMovementY) {
        PacketEntity entity = player.inVehicle() ? player.compensatedEntities.self.getRiding() : player.compensatedEntities.self;
    ...
    public static float getBlockBounceRestitution(StateType type, LimeyPlayer player) {
    ...
    public static double getBlockCollisionHeight(LimeyPlayer player, WrappedBlockState block) {
