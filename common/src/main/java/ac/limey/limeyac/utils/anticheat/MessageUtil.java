...
import ac.limey.limeyac.utils.data.webhook.discord.CompiledDiscordTemplate;
...
public class MessageUtil {
    ...
    public @Nullable String replacePlaceholders(@Nullable LimeyPlayer player, @Nullable String string, boolean removeFormatting) {
        return replacePlaceholders(player, player == null ? null : player.platformPlayer, string, removeFormatting);
    }

    @Contract("_, null -> null; _, !null -> !null")
    public @Nullable String replacePlaceholders(@Nullable LimeyPlayer player, @Nullable String string) {
    ...
    public @Nullable String replacePlaceholders(@Nullable PlatformPlayer player, @Nullable String string) {
        return replacePlaceholders(player == null ? null : LimeyAPI.INSTANCE.getPlayerDataManager().getPlayer(player.getUniqueId()), player, string, false);
    }
    ...
    private @Nullable String replacePlaceholders(@Nullable LimeyPlayer limeyPlayer, @Nullable PlatformPlayer platformPlayer, @Nullable String string, boolean removeFormatting) {
        ...
        if (!matcher.find()) {
            return LimeyAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, string);
        }

        // Get references to the maps once, outside the loop.
        final Map<String, String> staticReplacements = LimeyAPI.INSTANCE.getExternalAPI().getStaticReplacements();
        final Map<String, Function<GrimUser, String>> variableReplacements = LimeyAPI.INSTANCE.getExternalAPI().getVariableReplacements();
        ...
        String limeyReplaced = sb.toString();

        return LimeyAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, limeyReplaced).replace(PLACEHOLDER_ESCAPE_CHAR, '%');
    }

    public @NotNull Component replacePlaceholders(@NotNull LimeyPlayer player, @NotNull Component component) {
    ...
    public @NotNull Component miniMessage(@NotNull String string, TagResolver... resolvers) {
        string = string.replace("%prefix%", LimeyAPI.INSTANCE.getConfigManager().getConfig().getStringElse("prefix", "&bLimey &8»"));
    ...
    public Component getParsedComponent(Sender sender, String key, String fallbackText) {
        String message = LimeyAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallbackText);
