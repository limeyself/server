package ac.limey.limeyac.utils.collisions.blocks.connecting;
...
public class DynamicHitboxWall extends DynamicConnecting implements HitBoxFactory {
    @Override
    public CollisionBox fetch(LimeyPlayer player, StateType heldItem, ClientVersion version, WrappedBlockState state, boolean isTargetBlock, int x, int y, int z) {
    ...
    private int[] getConnections(LimeyPlayer player, ClientVersion version, WrappedBlockState state, int x, int y, int z) {
    ...
    @Override
    public boolean checkCanConnect(LimeyPlayer player, WrappedBlockState state, StateType one, StateType two, BlockFace direction) {
