...
import ac.limey.limeyac.utils.data.tags.SyncedTag;
import ac.limey.limeyac.utils.data.tags.SyncedTags;
import ac.limey.limeyac.utils.enums.FluidTag;
import ac.limey.limeyac.utils.inventory.EnchantmentHelper;
...
public class BlockBreakSpeed {
    // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/LimeyAnticheat/Limey/issues/2117
    ...
    // another temporary hardcode for the same reasons as above; see https://github.com/LimeyAnticheat/Limey/issues/2574
    ...
    public static boolean couldInstantlyBreakBlock(LimeyPlayer player, StateType block) {
        for (int i = 0; i < 9; i++) {
    ...
    public static double getBlockDamage(LimeyPlayer player, WrappedBlockState block) {
        ItemStack tool = player.inventory.getHeldItem();
        return getBlockDamage(player, tool, block.getType());
    }

    public static double getBlockDamage(LimeyPlayer player, ItemStack tool, StateType block) {
        // GET destroy speed
        ...
        final boolean canHarvest = !block.isRequiresCorrectTool() || toolSpeedData.isCorrectToolForDrop
                // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/LimeyAnticheat/Limey/issues/2091
                || player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_4) && HARVESTABLE_TYPES_1_21_4.contains(block)
                // same reason as above; see https://github.com/LimeyAnticheat/Limey/issues/2574
    ...
    private static float getSpeedMultiplierFromToolData(LimeyPlayer player, ItemStack tool, ToolSpeedData data) {
    ...
    private static ToolSpeedData getModernToolSpeedData(LimeyPlayer player, ItemStack tool, StateType block) {
    ...
    private static ToolSpeedData getLegacyToolSpeedData(LimeyPlayer player, ItemStack tool, StateType block) {
        ItemType toolType = tool.getType();
        float speedMultiplier = 1.0f;
        boolean isCorrectToolForDrop = false;
