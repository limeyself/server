...
import ac.limey.limeyac.utils.collisions.HitboxData;
...
import ac.limey.limeyac.utils.data.HitData;
...
public class WorldRayTrace {
    public static HitData getNearestBlockHitResult(LimeyPlayer player, StateType heldItem, boolean sourcesHaveHitbox, boolean fluidPlacement, boolean itemUsePlacement) {
    ...
    public static HitData traverseBlocks(LimeyPlayer player, Vector3d start, Vector3d end, BiFunction<WrappedBlockState, Vector3i, HitData> predicate) {
        // I guess go back by the collision epsilon?
        double endX = LimeyMath.lerp(-1.0E-7D, end.x, start.x);
        double endY = LimeyMath.lerp(-1.0E-7D, end.y, start.y);
        double endZ = LimeyMath.lerp(-1.0E-7D, end.z, start.z);
        double startX = LimeyMath.lerp(-1.0E-7D, start.x, end.x);
        double startY = LimeyMath.lerp(-1.0E-7D, start.y, end.y);
        double startZ = LimeyMath.lerp(-1.0E-7D, start.z, end.z);
        int floorStartX = LimeyMath.floor(startX);
        int floorStartY = LimeyMath.floor(startY);
        int floorStartZ = LimeyMath.floor(startZ);
        ...
        double d12 = posXInverse * (xSign > 0 ? 1.0D - LimeyMath.frac(startX) : LimeyMath.frac(startX));
        double d13 = posYInverse * (ySign > 0 ? 1.0D - LimeyMath.frac(startY) : LimeyMath.frac(startY));
        double d14 = posZInverse * (zSign > 0 ? 1.0D - LimeyMath.frac(startZ) : LimeyMath.frac(startZ));
