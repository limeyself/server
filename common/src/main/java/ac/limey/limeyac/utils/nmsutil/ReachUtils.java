...
public class ReachUtils {
    ...
    @Contract("_, _, _ -> new")
    public static @NotNull Vector3dm getLook(@NotNull LimeyPlayer player, float yaw, float pitch) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_12_2)) {
            float yawRadians = LimeyMath.radians(-yaw) - (float) Math.PI;
            float pitchRadians = LimeyMath.radians(-pitch);
    ...
    public static double getMinReachToBox(@NotNull LimeyPlayer player, @NotNull SimpleCollisionBox targetBox) {
