...
import ac.limey.limeyac.utils.blockstate.helper.BlockFaceHelper;
...
import ac.limey.limeyac.utils.nmsutil.Dripstone;
...
public enum BlockPlaceResult {
    ...
    TORCH_OR_HEAD((player, place) -> {
        // type doesn't matter to limey, same hitbox.
    ...
    // As long as one of these properties is met, it is good enough for limey!
    VINE((player, place) -> {
    ...
    SCAFFOLDING((player, place) -> {
        ...
        } // else, cancel if the scaffolding is exactly 7 away, limey doesn't handle this edge case yet.
