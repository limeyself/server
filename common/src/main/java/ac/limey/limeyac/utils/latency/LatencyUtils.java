...
public class LatencyUtils {
    ...
    public LatencyUtils(LimeyPlayer player) {
        this.player = player;
    }
        ...
        synchronized (this) {
            ...
            for (Runnable runnable : tasksToRun) {
                try {
                    ...
                    if (CommonLimeyArguments.KICK_ON_TRANSACTION_ERRORS.value()) {
                        player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, LimeyAPI.INSTANCE.getConfigManager().getDisconnectPacketError())));
                    }
                }
            }
        }
