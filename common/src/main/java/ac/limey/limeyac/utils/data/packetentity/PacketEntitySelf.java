...
import ac.limey.limeyac.checks.impl.sprint.SprintD;
...
public class PacketEntitySelf extends PacketEntity {
    ...
    public PacketEntitySelf(LimeyPlayer player) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    }

    public PacketEntitySelf(LimeyPlayer player, PacketEntitySelf old) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    ...
    protected void initAttributes(LimeyPlayer player) {
        ...
        trackAttribute(ValuedAttribute.ranged(Attributes.SNEAKING_SPEED, 0.3, 0, 1)
                .withGetRewriter(value -> {
                    ...
                    final double clamped = LimeyMath.clamp(0.3f + swiftSneak * 0.15f, 0f, 1f);
                    if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) {
                        return clamped;
                    }
