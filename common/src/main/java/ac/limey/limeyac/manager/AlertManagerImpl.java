...
public final class AlertManagerImpl implements AlertManager, ConfigReloadable, StartableInitable {
    ...
    @Override
    public void start() {
        platformServer = LimeyAPI.INSTANCE.getPlatformServer();
     ...
     * @throws IllegalArgumentException if the user is not a LimeyPlayer.
     * @throws NullPointerException if the LimeyPlayer's platformPlayer is null.
     */
    private @NotNull PlatformPlayer requirePlatformPlayerFromUser(@NotNull GrimUser user) {
        ...
        if (!(user instanceof LimeyPlayer limeyPlayer)) {
            // Throw a specific exception if the type is wrong
            throw new IllegalArgumentException("AlertManager action called with non-LimeyPlayer user: " + user.getName());
        }

        PlatformPlayer platformPlayer = limeyPlayer.platformPlayer;
    ...
    public boolean hasBrandsEnabled(@NotNull GrimUser player) {
        LimeyPlayer limeyPlayer = (LimeyPlayer) player;
        ...
        if (limeyPlayer.platformPlayer == null) return false;

        return hasBrandsEnabled(limeyPlayer.platformPlayer);
    }
    ...
    public boolean hasAlertsEnabled(Player player) {
        ...
        return hasAlertsEnabled(LimeyAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
    ...
    public void toggleAlerts(Player player) {
        ...
        toggleAlerts(LimeyAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
    ...
    public boolean hasVerboseEnabled(Player player) {
        if (player == null) return false;
        return hasVerboseEnabled(LimeyAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
    ...
    public void toggleVerbose(Player player) {
        if (player == null) return;
        toggleVerbose(LimeyAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
