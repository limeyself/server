...
import ac.limey.limeyac.manager.player.features.types.*;
...
public class FeatureManagerImpl implements FeatureManager, ConfigReloadObserver {
    ...
    private static final Map<String, LimeyFeature> FEATURES;

    /// @deprecated use {@link #getFeatures()}
    @Contract(pure = true)
    @Deprecated
    public static Map<String, LimeyFeature> getFEATURES() {
        return getFeatures();
    }
    ...
    public FeatureManagerImpl(LimeyPlayer player) {
        this.player = player;
        for (LimeyFeature value : FEATURES.values()) states.put(value.getName(), FeatureState.UNSET);
    }
    ...
    public boolean isFeatureEnabled(String key) {
        LimeyFeature feature = FEATURES.get(key);
    ...
    @Override
    public void reload() {
        onReload(LimeyAPI.INSTANCE.getExternalAPI().getConfigManager());
    }
