...
import ac.limey.limeyac.events.packets.ProxyAlertMessenger;
...
public class PunishmentManager implements ConfigReloadable {
    private static final CommandExecuteEvent.Channel COMMAND_CHANNEL = LimeyAPI.INSTANCE.getEventBus().get(CommandExecuteEvent.class);
    private final LimeyPlayer player;
    ...
    public PunishmentManager(LimeyPlayer player) {
        this.player = player;
    }
    ...
    public boolean handleAlert(LimeyPlayer player, String verbose, Check check) {
        String value = verbose == null ? "" : verbose;
        return handleAlert(player, () -> value, check);
    }

    public boolean handleAlert(LimeyPlayer player, Supplier<String> verbose, Check check) {
        ...
        for (PunishGroup group : groups) {
            if (group.checks.contains(check)) {
                ...
                for (ParsedCommand command : group.commands) {
                    ...
                    // Verbose that prints all flags: /limey verbose subscribers get EVERY flag, not just thresholded
                    ...
                    if (command.command.equals("[alert]") && LimeyAPI.INSTANCE.getAlertManager().hasVerboseListeners()) {
                        sentDebug = true;
                        ...
                        verboseListeners = LimeyAPI.INSTANCE.getAlertManager().sendVerbose(MessageUtil.miniMessage(listenerCmd), null);
                    }
                    if (violationCount >= command.threshold) {
                        ...
                        if (shouldRun) {
                            ...
                            switch (command.command) {
                                case "[webhook]" -> LimeyAPI.INSTANCE.getDiscordManager().sendAlert(player, renderedVerbose, check.getDisplayName(), vl);
                                case "[log]" -> {
                                ...
                                case "[alert]" -> {
                                    ...
                                    if (testMode) { // secret test mode
                                        ...
                                        LimeyAPI.INSTANCE.getAlertManager().sendAlert(message, verboseListeners);
                                    }
                                }
                                default -> LimeyAPI.INSTANCE.getScheduler().getGlobalRegionScheduler().run(LimeyAPI.INSTANCE.getLimeyPlugin(), () ->
                                        LimeyAPI.INSTANCE.getPlatformServer().dispatchCommand(
