...
import ac.limey.limeyac.utils.data.webhook.discord.Embed;
import ac.limey.limeyac.utils.data.webhook.discord.EmbedField;
import ac.limey.limeyac.utils.data.webhook.discord.EmbedFooter;
...
public class DiscordManager implements StartableInitable, ReloadableInitable {
    ...
    private static final Duration timeout = Duration.ofMillis(CommonLimeyArguments.URL_TIMEOUT.value());
    ...
    private static String validatedConfigURL(String configPath, String defaultURL) {
        String url = LimeyAPI.INSTANCE.getConfigManager().getConfig().getStringElse("embed-image-url", defaultURL);
    ...
    public void reload() {
        try {
            ...
            ConfigManager config = LimeyAPI.INSTANCE.getConfigManager().getConfig();
            ...
            embedFooterUrl = validatedConfigURL("embed-footer-url", "https://limey.ac/images/limey.png");
            embedFooterText = config.getStringElse("embed-footer-text", "v%limey_version%");
            embedTitle = config.getStringElse("embed-title", "**Limey Alert**");
    ...
    public void sendAlert(@NotNull LimeyPlayer player, String verbose, String checkName, int violations) {
        if (isDisabled()) {
            return;
        }

        // Per-alert overlay — avoids polluting the global static map
        Map<String, String> statics = new HashMap<>(LimeyAPI.INSTANCE.getExternalAPI().getStaticReplacements());
        statics.put("%check%", checkName);
        statics.put("%violations%", Integer.toString(violations));

        Map<String, Function<GrimUser, String>> dynamics = LimeyAPI.INSTANCE.getExternalAPI().getVariableReplacements();
    ...
    public CompletableFuture<Boolean> sendWebhookMessage(WebhookMessage message) {
        ...
        if (!taskStarted.getAndSet(true)) {
            ...
            LimeyAPI.INSTANCE.getScheduler().getAsyncScheduler().runAtFixedRate(LimeyAPI.INSTANCE.getLimeyPlugin(), DiscordManager::tick, 0, 1);
        }
