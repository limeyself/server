...
public interface LiveWriteHooks {
    ...
    /** Convenience for {@code PacketPlayerJoinQuit.onUserLogin}. Builds meta from the User + LimeyPlayer internally. */
    void onJoinFromUserLogin(@NotNull PlatformPlayer player, @NotNull User user, long now);

    /** Convenience for {@code PlayerDataManager.onDisconnect}. Builds meta internally; tolerates null limeyPlayer (early-disconnect race). */
    void onQuitFromUserDisconnect(@NotNull User user, @Nullable LimeyPlayer limeyPlayer, long now);

    /** Convenience for {@code ClientBrand.onPacketReceive}. Pulls UUID + builds meta from the LimeyPlayer internally. */
    void observeBrandFromCheck(@NotNull LimeyPlayer limeyPlayer);
    ...
    /** Binary-verbose counterpart to {@link #recordFlagFromCheck(LimeyPlayer, AbstractCheck, double, String)}. */
     ...
     * for {@code user}. Reads from the passed-in {@link LimeyPlayer} (when
    ...
    static @NotNull SessionTracker.ClientMeta clientMetaFor(
            @NotNull User user, @Nullable LimeyPlayer gp) {
        String limeyVersion = LimeyAPI.INSTANCE.getExternalAPI().getGrimVersion();
        int clientVersionPvn = user.getClientVersion().getProtocolVersion();
        String serverVersion = LimeyAPI.INSTANCE.getPlatformServer().getPlatformImplementationString();
        String brand = null;
        if (gp != null) {
            ClientBrand check = gp.checkManager.getCheck(ClientBrand.class);
            if (check != null && check.isHasBrand()) brand = check.getBrand();
        }
        return new SessionTracker.ClientMeta(limeyVersion, brand, clientVersionPvn, serverVersion);
    }
    ...
    LiveWriteHooks NOOP = new LiveWriteHooks() {
        ...
        @Override public void onQuitFromUserDisconnect(@NotNull User u, @Nullable LimeyPlayer g, long t) {}
        @Override public void observeBrandFromCheck(@NotNull LimeyPlayer g) {}
        @Override public void recordFlagFromCheck(@NotNull LimeyPlayer p, @NotNull AbstractCheck c, double v, @Nullable String vb) {}
        @Override public void recordFlagDataFromCheck(@NotNull LimeyPlayer p, @NotNull AbstractCheck c, double v, byte @Nullable [] d) {}
    };
}
