...
public class AttackCooldownHandler extends Check implements PacketReceiveListener {
    ...
    public AttackCooldownHandler(LimeyPlayer player) {
        super(player);
    }
    ...
    public float getMinimumProgress() {
        return LimeyMath.clamp(((float) ticksSinceLastSwing + 0.5F) / (float) (1d / player.compensatedEntities.self.getAttributeValue(Attributes.ATTACK_SPEED) * 20.0D), 0, 1);
    }
}
