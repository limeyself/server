...
import ac.limey.limeyac.checks.impl.badpackets.BadPacketsN;
...
import ac.limey.limeyac.utils.nmsutil.ReachUtils;
...
public class SetbackTeleportUtil extends Check implements PostPredictionListener {
    ...
    private static final GrimTeleportEvent.Channel TELEPORT_CHANNEL = LimeyAPI.INSTANCE.getEventBus().get(GrimTeleportEvent.class);
    private static final GrimPlayerSetbackEvent.Channel PLAYER_SETBACK_CHANNEL = LimeyAPI.INSTANCE.getEventBus().get(GrimPlayerSetbackEvent.class);
    ...
    public SetbackTeleportUtil(LimeyPlayer player) {
        super(player);
    }
    ...
    public void executeForceResync() {
        if (player.gamemode == GameMode.SPECTATOR || player.disableLimey)
    ...
    private boolean isExempt() {
        ...
        if (player.disableLimey) return true;
        ...
        try {
            ...
            if (player.inVehicle()) {
                ...
                if (player.compensatedEntities.serverPlayerVehicle != null) {
                    ...
                    LimeyAPI.INSTANCE.getScheduler().getEntityScheduler().execute(player.platformPlayer, LimeyAPI.INSTANCE.getLimeyPlugin(), () -> {
                        if (player.platformPlayer != null) {
                            LimeyEntity vehicle = player.platformPlayer.getVehicle();
                            if (vehicle != null) {
                                vehicle.eject();
                            }
                        }
            ...
            if (PacketEvents.getAPI().getServerManager().getVersion().isOlderThanOrEquals(ServerVersion.V_1_7_10)) {
                y += 1.62; // 1.7 teleport offset if limey ever supports 1.7 again
            }
    ...
    public boolean insideUnloadedChunk() {
        Column column = player.compensatedWorld.getChunk(LimeyMath.floor(player.x) >> 4, LimeyMath.floor(player.z) >> 4);

        // If true, the player is in an unloaded chunk
        return !player.disableLimey && (column == null || column.transaction() >= player.lastTransactionReceived.get() ||
