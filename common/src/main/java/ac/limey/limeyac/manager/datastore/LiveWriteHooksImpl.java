...
public final class LiveWriteHooksImpl implements LiveWriteHooks {
    ...
    public void onJoinFromUserLogin(@NotNull PlatformPlayer player, @NotNull User user, long now) {
        LimeyPlayer gp = LimeyAPI.INSTANCE.getPlayerDataManager().getPlayer(user);
        onJoin(player.getUniqueId(), player.getName(), now, LiveWriteHooks.clientMetaFor(user, gp));
    }

    @Override
    public void onQuitFromUserDisconnect(@NotNull User user, @Nullable LimeyPlayer limeyPlayer, long now) {
        UUID uuid = user.getUUID();
        if (uuid == null) return; // disconnected pre-LOGIN_SUCCESS — no session to close
        onQuit(uuid, now, LiveWriteHooks.clientMetaFor(user, limeyPlayer));
    }

    @Override
    public void observeBrandFromCheck(@NotNull LimeyPlayer limeyPlayer) {
        UUID uuid = limeyPlayer.user.getUUID();
        if (uuid == null) return;
        observeBrand(uuid, System.currentTimeMillis(), LiveWriteHooks.clientMetaFor(limeyPlayer.user, limeyPlayer));
    }
    ...
    private int resolveCheckId(@NotNull AbstractCheck check) {
        ...
        if (declaredStable != null && !declaredStable.isEmpty()) {
            ...
            if (missingStableKeyLogged.add(key)) {
                LogUtil.warn("[limey-history] check " + display
