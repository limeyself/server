...
public class LimeyDump implements BuildableCommand {
    ...
    private void handleDump(@NotNull CommandContext<Sender> context) {
        ...
        if (link != null) {
            sender.sendMessage(MessageUtil.miniMessage(LimeyAPI.INSTANCE.getConfigManager().getConfig()
                    .getStringElse("upload-log", "%prefix% &fUploaded debug to: %url%")
                    .replace("%url%", link)));
            return;
        }
        // TODO: change this back to application/json once allowed
        LimeyLog.sendLogAsync(sender, generateDump(), string -> link = string, "text/yaml");
    }
    ...
    public static JsonObject getDumpInfo() {
        ...
        versions.addProperty("limey", LimeyAPI.INSTANCE.getExternalAPI().getGrimVersion());
        ...
        versions.addProperty("implementation", LimeyAPI.INSTANCE.getPlatformServer().getPlatformImplementationString());
        // state of different properties
        JsonObject states = new JsonObject();
        base.add("states", states);
        if (LimeyAPI.INSTANCE.isInitialized()) states.addProperty("platform", LimeyAPI.INSTANCE.getPlatform().toString());
        ...
        if (LimeyAPI.INSTANCE.getAlertManager().hasConsoleVerboseEnabled()) settings.addProperty("console_verbose", true);
        if (!LimeyAPI.INSTANCE.getAlertManager().hasConsoleAlertsEnabled()) settings.addProperty("console_alerts", false);
        ...
        for (PlatformPlugin plugin : LimeyAPI.INSTANCE.getPluginManager().getPlugins()) {
    ...
    private static JsonObject getBuildInfo() {
        ...
        try {
            Properties properties = PropertiesUtil.readProperties(LimeyAPI.INSTANCE.getClass(), "limey.properties");
