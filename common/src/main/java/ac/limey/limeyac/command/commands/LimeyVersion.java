...
public class LimeyVersion implements BuildableCommand {

    private static final AtomicReference<Component> updateMessage = new AtomicReference<>();
    private static final HttpClient HTTP_CLIENT = HttpClient.newBuilder()
            .connectTimeout(Duration.of(CommonLimeyArguments.URL_TIMEOUT.value(), ChronoUnit.MILLIS))
            .build();
    private static long lastCheck;

    public static void checkForUpdatesAsync(Sender sender) {
        String current = LimeyAPI.INSTANCE.getExternalAPI().getGrimVersion();
        sender.sendMessage(Component.text()
                .append(Component.text("Limey Version: ").color(NamedTextColor.GRAY))
        ...
        lastCheck = now;
        LimeyAPI.INSTANCE.getScheduler().getAsyncScheduler().runNow(LimeyAPI.INSTANCE.getLimeyPlugin(), () -> checkForUpdates(sender));
    }
    ...
    private static void checkForUpdates(Sender sender) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(CommonLimeyArguments.API_URL.value() + "updates"))
                    .GET()
                    .header("User-Agent", "Limey/" + LimeyAPI.INSTANCE.getExternalAPI().getGrimVersion())
                    .header("Content-Type", "application/json")
                    .timeout(Duration.of(CommonLimeyArguments.URL_TIMEOUT.value(), ChronoUnit.MILLIS))
                    .build();
            ...
            if (statusCode < 200 || statusCode >= 300) {
                ...
                sender.sendMessage(Objects.requireNonNullElseGet(msg, () -> Component.text()
                        ...
                        .append(Component.text(" Failed to check latest Limey version. Update server responded with status code: ")
            ...
            if (object.has("status")) {
                ...
                status = Status.SemVer.getVersionStatus(LimeyAPI.INSTANCE.getExternalAPI().getGrimVersion(), latest);
            }
            //
            Component msg = switch (status) {
                case AHEAD ->
                        Component.text("You are using a development version of Limey").color(NamedTextColor.LIGHT_PURPLE);
                case UPDATED ->
                        Component.text("You are using the latest version of Limey").color(NamedTextColor.GREEN);
                case OUTDATED -> Component.text()
                        .append(Component.text("New Limey version found!").color(NamedTextColor.AQUA))
                        ...
                        .build();
                case UNKNOWN ->
                        Component.text("You are using an unknown Limey version.").color(NamedTextColor.RED);
            };
            ...
            LogUtil.error("Failed to check latest Limey version.", e);
        }
    }
