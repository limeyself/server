...
import ac.limey.limeyac.command.CommandUtils;
...
public class LimeyList implements BuildableCommand {

    // Mainly for debugging purposes. Useful for seeing which players are exempt or not.

    @Override
    public void register(CommandManager<Sender> commandManager, CloudPlatformCommandArguments arguments) {
        commandManager.command(commandManager.commandBuilder("limey", "limey")
                .literal("list")
                .permission("limey.list")
    ...
    private void handleListPlayers(Sender sender) {
        ...
        final Map<UUID, PlatformPlayer> onlinePlayers = LimeyAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers()
        ...
        //
        for (LimeyPlayer entry : LimeyAPI.INSTANCE.getPlayerDataManager().getEntries()) {
            if (after) {
            ...
            boolean exempt = !LimeyAPI.INSTANCE.getPlayerDataManager().shouldCheck(entry.user);
