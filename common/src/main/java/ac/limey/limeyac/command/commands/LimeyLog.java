...
public class LimeyLog implements BuildableCommand {
    public static void sendLogAsync(Sender sender, String log, Consumer<String> consumer, String type) {
        String success = LimeyAPI.INSTANCE.getConfigManager().getConfig().getStringElse("upload-log", "%prefix% &fUploaded debug to: %url%");
        String failure = LimeyAPI.INSTANCE.getConfigManager().getConfig().getStringElse("upload-log-upload-failure", "%prefix% &cSomething went wrong while uploading this log, see console for more information.");
        String uploading = LimeyAPI.INSTANCE.getConfigManager().getConfig().getStringElse("upload-log-start", "%prefix% &fUploading log... please wait");
    ...
    private static void sendLog(Sender sender, String log, String success, String failure, Consumer<String> consumer, String type) throws IOException {
        URL mUrl = new URL(CommonLimeyArguments.PASTE_URL.value() + "data/post");
        HttpURLConnection urlConn = (HttpURLConnection) mUrl.openConnection();
        try {
            urlConn.setDoOutput(true);
            urlConn.setRequestMethod("POST");
            urlConn.setConnectTimeout(CommonLimeyArguments.URL_TIMEOUT.value());
            urlConn.setReadTimeout(CommonLimeyArguments.URL_TIMEOUT.value());
            urlConn.addRequestProperty("User-Agent", "Limey/" + LimeyAPI.INSTANCE.getExternalAPI().getGrimVersion());
            ...
            if (response == HttpURLConnection.HTTP_CREATED) {
                ...
                String message = success.replace("%url%", CommonLimeyArguments.PASTE_URL.value() + responseURL);
    ...
    public void register(CommandManager<Sender> commandManager, CloudPlatformCommandArguments arguments) {
        Command<Sender> command = commandManager.commandBuilder("limey", "limey")
                .literal("log", "logs")
                .permission("limey.log")
