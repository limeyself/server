package ac.limey.limeyac.command.render;
...
public final class HistoryComponentRenderer {
    ...
    private static final String SESSION_ROW_FALLBACK = "<hover:show_text:'&bSession &7%session_uuid%"
            ...
            + "&8[&b%limey_version%&8] &8[&b%server_name%&8] &8[&b%client_version%&8]"
     ...
     * disambiguated {@code /limey history player <target>} form so a shared
    ...
    public static @NotNull List<Component> renderSessionList(
        ...
        ConfigManager cfg = LimeyAPI.INSTANCE.getConfigManager().getConfig();
        String safePlayer = mmSafe(playerDisplayName);
        if (result.items().isEmpty()) {
            return List.of(parse(sender, cfg, "limey-history-no-sessions",
        ...
        // operators upgrading from 1.x with a legacy limey-history-header that
        // still references %maxPages% see a correctly-rendered count.
        String maxPagesStr = Integer.toString(Math.max(1, maxPages));
        out.add(parse(sender, cfg, "limey-history-header",
    ...
    private static Component renderSummaryLine(Sender sender, ConfigManager cfg, SessionSummary s, boolean ongoing,
        ...
        String crashedMarker = (!ongoing && s.endedUnexpectedly())
                ? cfg.getStringElse("limey-history-crashed-marker", " &8(&ccrashed&8)")
                : "";
        String detailCommand = "/limey history player " + playerDisplayName + " session " + s.sessionOrdinal();
        RenderedTemplate rendered = parse(sender, cfg, "limey-history-session", SESSION_ROW_FALLBACK,
                Map.ofEntries(
                        ...
                        Map.entry("limey_version", TemplateValue.raw(nullToUnknown(s.grimVersion()))),
    ...
    public static @NotNull List<Component> renderSessionDetail(
        ...
        Map<String, TemplateValue> metaVars = Map.ofEntries(
                ...
                Map.entry("limey_version", TemplateValue.raw(nullToUnknown(d.grimVersion()))),
        ...
        out.add(parse(sender, cfg, "limey-history-detail-header",
                "%prefix% &bShowing &f%player%&b's session &f%ordinal%&b details:", metaVars).component());
        out.add(parse(sender, cfg, "limey-history-detail-meta1",
                "&bLimey: &f%limey_version%&b, Server: &f%server_name%&b, Duration: &f%duration%&b, Date: &7%timeago% ago",
                metaVars).component());
        out.add(parse(sender, cfg, "limey-history-detail-meta2",
                "&bClient: &f%client_version%&b, Brand: &f%client_brand%",
                metaVars).component());
        out.add(parse(sender, cfg, "limey-history-detail-violations-header",
                "&bViolations: &8(%violations% total, %unique_checks% unique) &8[&f%page%&7/&f%max_pages%&8]",
                metaVars).component());

        if (d.violations().isEmpty()) {
            out.add(parse(sender, cfg, "limey-history-detail-empty", "&7- (none)", Map.of()).component());
            return out;
        }
        ...
        if (detailed) {
            ...
            // joining per-check entries rendered via limey-history-check-count.
    ...
    private static Component renderBucketLine(Sender sender, ConfigManager cfg, CheckBucket bucket, SessionDetail d, boolean verbose) {
        ...
        for (CheckCount c : bucket.checks()) {
            ...
            checksList.append(cfg.getStringElse("limey-history-check-count",
        ...
        RenderedTemplate rendered = parse(sender, cfg, "limey-history-detail-group", GROUP_ROW_FALLBACK,
                Map.of(
    ...
    private static Component buildBucketHoverEntries(Sender sender, ConfigManager cfg, SessionDetail d, CheckBucket bucket, boolean verbose) {
        ...
        for (ViolationEntry v : d.violations()) {
            ...
            Component description = v.description().isBlank()
                    ? Component.empty()
                    : parse(sender, cfg, "limey-history-hover-description", " — &f%description%",
            ...
            Component verboseComponent = verbose && !verboseText.isBlank()
                    ? parse(sender, cfg, "limey-history-detail-group-hover-verbose", " — &7%verbose%",
                            Map.of("verbose", TemplateValue.text(verboseText))).component()
                    : Component.empty();
            body = body.append(parse(sender, cfg, "limey-history-detail-group-hover-entry",
    ...
    private static Component bucketHover(Sender sender, ConfigManager cfg, String bucketStart, String bucketEnd,
        ...
        return parse(sender, cfg, "limey-history-detail-group-hover",
    ...
    private static Component renderViolationLine(Sender sender, ConfigManager cfg, ViolationEntry v, boolean verbose) {
        ...
        RenderedTemplate rendered = parse(sender, cfg, "limey-history-detail-entry",
    ...
    private static Component violationHover(Sender sender, ConfigManager cfg, ViolationEntry v,
        ...
        Component verbose = !verboseText.isBlank()
                ? parse(sender, cfg, "limey-history-detail-entry-hover-verbose", "<newline>&7%verbose%",
                        Map.of("verbose", TemplateValue.text(verboseText))).component()
                : Component.empty();
        return parse(sender, cfg, "limey-history-detail-entry-hover",
     ...
     *
     * <p>Used by {@code /limey history --name <regex>} / {@code --match
