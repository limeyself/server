...
@CheckData(name = "Simulation", stableKey = "limey.prediction.simulation", description = "Moved differently than predicted movement simulation", decay = 0.02)
public class OffsetHandler extends Check implements PostPredictionListener {
    ...
    private static final CompletePredictionEvent.Channel COMPLETE_CHANNEL = LimeyAPI.INSTANCE.getEventBus().get(CompletePredictionEvent.class);

    public OffsetHandler(LimeyPlayer player) {
        super(player);
    }
