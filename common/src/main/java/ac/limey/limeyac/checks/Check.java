...
@Getter
public class Check extends LimeyProcessor implements AbstractCheck {
    private static final FlagEvent.Channel FLAG_CHANNEL = LimeyAPI.INSTANCE.getEventBus().get(FlagEvent.class);

    protected final @NotNull LimeyPlayer player;
    ...
    public Check(final @NotNull LimeyPlayer player) {
    ...
    public final void updatePermissions() {
        ...
        exemptPermission = player.hasPermission("limey.exempt." + id);
        noSetbackPermission = player.hasPermission("limey.nosetback." + id);
        noModifyPacketPermission = player.hasPermission("limey.nomodifypacket." + id);
    }
    ...
    private boolean recordFlag(@NotNull Supplier<String> verbose) {
        if (player.disableLimey || (experimental && !player.isExperimentalChecks()) || exemptPermission)
