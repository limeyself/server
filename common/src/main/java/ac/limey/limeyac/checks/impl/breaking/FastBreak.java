...
@CheckData(name = "FastBreak", stableKey = "limey.breaking.fast_break", description = "Breaking blocks too quickly")
public class FastBreak extends Check implements BlockBreakListener, PreViaPacketReceiveListener {
    ...
    public FastBreak(LimeyPlayer playerData) {
        super(playerData);
    }
    ...
    private void clampBalance() {
        ...
        blockBreakBalance = LimeyMath.clamp(blockBreakBalance, -balance, balance); // Clamp not Math.max in case other logic changes
        blockDelayBalance = LimeyMath.clamp(blockDelayBalance, -balance, balance);
    }
}
