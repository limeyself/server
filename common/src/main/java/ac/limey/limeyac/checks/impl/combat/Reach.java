// This file was designed and is an original check for Limey
...
import ac.limey.limeyac.utils.data.packetentity.PacketEntitySizeable;
import ac.limey.limeyac.utils.data.packetentity.dragon.PacketEntityEnderDragonPart;
...
@CheckData(name = "Reach", stableKey = "limey.combat.reach", description = "Attacked an entity from too far away")
public class Reach extends Check implements PacketReceiveListener {
    ...
    public Reach(LimeyPlayer player) {
        super(player);
    }

    @Override
    public void onPacketReceive(final PacketReceiveEvent event) {
        if (!player.disableLimey && event.getPacketType() == PacketType.Play.Client.ATTACK) {
        ...
        if (!player.disableLimey && event.getPacketType() == PacketType.Play.Client.INTERACT_ENTITY) {
    ...
    private void onInteract(PacketReceiveEvent event, int entityId, InteractionHand hand) {
        ...
        // Dead entities cause false flags (https://github.com/LimeyAnticheat/Limey/issues/546)
