...
public class ClientBrand extends Check implements PacketReceiveListener {
    ...
    public ClientBrand(LimeyPlayer player) {
        super(player);
    }
    ...
    private void handle(String channel, byte[] data) {
        ...
        if (data.length > 64 || data.length == 0) {
            ...
            if (!LimeyAPI.INSTANCE.getConfigManager().isIgnoredClient(brand)) {
                String message = LimeyAPI.INSTANCE.getConfigManager().getConfig().getStringElse("client-brand-format", "%prefix% &f%player% joined using %brand%");
                Component component = MessageUtil.replacePlaceholders(player, MessageUtil.miniMessage(message));

                LimeyAPI.INSTANCE.getAlertManager().sendBrand(component, null);
            }
            ...
            LimeyAPI.INSTANCE.getDataStoreLifecycle().liveWriteHooks().observeBrandFromCheck(player);
        }
        ...
        if (hasReachHacks && LimeyAPI.INSTANCE.getConfigManager().isBlockBlacklistedForgeClients()) {
            player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, LimeyAPI.INSTANCE.getConfigManager().getDisconnectBlacklistedForge())));
        }
