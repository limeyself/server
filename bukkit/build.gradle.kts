...
dependencies {
    ...
    implementation(libs.limey.bukkit.internal)
...
bukkit {
    name = "Limey"
    author = "Limey"
    main = "ac.limey.limeyac.platform.bukkit.LimeyBukkitLoaderPlugin"
    website = "https://limey.ac/"
    apiVersion = "1.13"
    foliaSupported = true
    ...
    permissions {
        register("limey.alerts") {
            description = "Receive alerts for violations"
            default = Permission.Default.OP
        }

        register("limey.alerts.enable-on-join") {
            description = "Enable alerts on join"
            default = Permission.Default.OP
        }

        register("limey.performance") {
            description = "Check performance metrics"
            default = Permission.Default.OP
        }

        register("limey.profile") {
            description = "Check user profile"
            default = Permission.Default.OP
        }

        register("limey.brand") {
            description = "Show client brands on join"
            default = Permission.Default.OP
        }

        register("limey.brand.enable-on-join") {
            description = "Enable showing client brands on join"
            default = Permission.Default.OP
        }

        register("limey.sendalert") {
            description = "Send cheater alert"
            default = Permission.Default.OP
        }

        register("limey.nosetback") {
            description = "Disable setback"
            default = Permission.Default.FALSE
        }

        register("limey.nomodifypacket") {
            description = "Disable modifying packets"
            default = Permission.Default.FALSE
        }

        register("limey.disabled") {
            description = "Disable Limey checks while keeping player state tracked"
            default = Permission.Default.FALSE
        }

        register("limey.exempt") {
            description = "Exempt from all checks"
            default = Permission.Default.FALSE
        }

        register("limey.verbose") {
            description = "Receive verbose alerts for violations"
            default = Permission.Default.OP
        }

        register("limey.verbose.enable-on-join") {
            description =
